#!/usr/bin/env python3
"""
10,000-step MD Simulation of DHFR with ANI-2x + Implicit Solvent

CORRECTED VERSION: Uses ANI-2x force field + GB implicit solvent

This demonstrates:
1. Loading DHFR structure (2,499 atoms)
2. Running 10,000-step MD simulation with ANI-2x + GB implicit solvent
3. Measuring performance with Phase 3C optimizations
...
# Force JAX to use CPU (CUDA handles GB, JAX handles ANI-2x)
jax.config.update('jax_platform_name', 'cpu')
jax.config.update("jax_enable_x64", True)

# Import FeNNol MD initialization functions
...
def main():
    print("=" * 70)
    print("DHFR 10,000-Step MD Simulation with ANI-2x + Implicit Solvent")
    print("=" * 70)
    print()

    # =====================================================================
    # SYSTEM SETUP - Using FeNNol MD interface
    # =====================================================================

    # Load DHFR structure
    xyz_file_path = Path("examples/md/dhfr/dhfr2_nowat.xyz")
    if not xyz_file_path.exists():
        print(f"Error: {xyz_file_path} not found")
        return 1

    print(f"Loading structure: {xyz_file_path}")

    # Check if ANI-2x model file exists
    ani_model_file = Path("examples/md/ani2x.fnx")
    if not ani_model_file.exists():
        print(f"Error: ANI-2x model file not found: {ani_model_file}")
        print("Please ensure ani2x.fnx is in examples/md/")
        return 1

    # Create parameters for FeNNol MD interface
    params = {
        "xyz_input/file": str(xyz_file_path),
        "xyz_input/indexed": True,  # Tinker XYZ has index column
        "xyz_input/has_comment_line": False,  # Tinker format has no comment line
        "model_file": str(ani_model_file),
    ...
    # Load model with ANI-2x + GB implicit solvent
    print("Loading ANI-2x model with GB implicit solvent...")
    try:
        ...
        print("✓ ANI-2x + GB implicit solvent loaded successfully")
        print()
    ...
    coords_jax = conformation["coordinates"]
    natoms = coords_jax.shape[0]
    print(f"  Atoms: {natoms}")
    print()

    # Get masses for MD integration
    # Read elements from XYZ file for mass assignment
    coords_np, elements = read_tinker_xyz(xyz_file_path)
    ...
    # Compute initial energy and forces using the combined model
    # The model includes both ANI-2x and GB, computed together
    energy_total, forces_jax, info = model.energy_and_forces(**conformation)
    forces = np.array(forces_jax)

    # Convert units: Hartree to kcal/mol, Hartree/Bohr to kcal/mol/Å
    hartree_to_kcal = 627.5095
    bohr_to_angstrom = 0.529177

    # Energy might be an array, extract scalar
    energy_total_scalar = float(jnp.sum(energy_total)) if hasattr(energy_total, '__len__') else float(energy_total)
    energy_total_kcal = energy_total_scalar * hartree_to_kcal
    forces_kcal = forces * (hartree_to_kcal / bohr_to_angstrom)

    print(f"  Initial energy: {energy_total_kcal:.4f} kcal/mol")
    print(f"  Max force: {np.abs(forces_kcal).max():.4f} kcal/mol/Å")
    print()
    ...
    print()
    print(f"{'Step':>6} {'Time':>8} {'E_total':>12} {'Temp':>8} {'Max_F':>10} {'Time/step':>12}")
    print("-" * 80)

    # Storage
    coords_np = np.array(coords_jax)
    trajectory = [coords_np.copy()]
    energies_total_list = [energy_total_kcal]
    temperatures_traj = [temp_initial]
    step_times = []

    # Current state (in Bohr, as FeNNol uses atomic units internally)
    coords_current_bohr = np.array(coords_jax)
    velocities_current = velocities.copy()

    # Convert coords to Angstrom for velocity Verlet
    coords_current_ang = coords_current_bohr * bohr_to_angstrom
    ...
    for step in range(nsteps):
        ...
        # Compute forces using combined ANI-2x + GB model
        # Update conformation with current coordinates
        conformation_current = conformation.copy()
        conformation_current["coordinates"] = jnp.array(coords_current_bohr)

        energy_total, forces_jax, info = model.energy_and_forces(**conformation_current)

        # Convert to kcal/mol units for MD integration
        energy_total_scalar = float(jnp.sum(energy_total)) if hasattr(energy_total, '__len__') else float(energy_total)
        energy_total_kcal = energy_total_scalar * hartree_to_kcal
        forces_kcal = np.array(forces_jax) * (hartree_to_kcal / bohr_to_angstrom)

        # Velocity Verlet step 1: update positions and half-step velocities
        coords_new_ang, velocities_half, accel_old = velocity_verlet_step(
            coords_current_ang, velocities_current, masses, forces_kcal, dt
        )

        # Convert back to Bohr for energy/force evaluation
        coords_new_bohr = coords_new_ang / bohr_to_angstrom

        # Compute forces at new position
        conformation_new = conformation.copy()
        conformation_new["coordinates"] = jnp.array(coords_new_bohr)

        energy_total_new, forces_jax_new, info = model.energy_and_forces(**conformation_new)

        # Convert units
        energy_total_new_scalar = float(jnp.sum(energy_total_new)) if hasattr(energy_total_new, '__len__') else float(energy_total_new)
        energy_total_new_kcal = energy_total_new_scalar * hartree_to_kcal
        forces_new_kcal = np.array(forces_jax_new) * (hartree_to_kcal / bohr_to_angstrom)

        # Velocity Verlet step 2: complete velocity update
        kcal_to_amu = 6.95e-4
        accel_new = forces_new_kcal * kcal_to_amu / masses[:, None]
        velocities_new = velocities_half + 0.5 * accel_new * dt

        # Update state
        coords_current_bohr = coords_new_bohr
        coords_current_ang = coords_new_ang
        ...
        if step % output_freq == 0 or step == nsteps - 1:
            ...
            max_force = np.abs(forces_new_kcal).max()

            print(f"{step+1:6d} {time_ps:8.3f} {energy_total_new_kcal:12.4f} {temp_current:8.2f} {max_force:10.4f} {step_time:12.2f}")

            # Store trajectory (in Angstrom)
            trajectory.append(coords_current_ang.copy())
            energies_total_list.append(energy_total_new_kcal)
    ...
    energies_total_arr = np.array(energies_total_list)
    temperatures_traj = np.array(temperatures_traj)

    print("Trajectory Analysis:")
    print(f"  Frames saved: {len(trajectory)}")
    print(f"  Energy (ANI-2x + GB):")
    print(f"    Mean: {np.mean(energies_total_arr):.4f} kcal/mol")
    print(f"    Std: {np.std(energies_total_arr):.4f} kcal/mol")
    print(f"    Min: {np.min(energies_total_arr):.4f} kcal/mol")
    print(f"    Max: {np.max(energies_total_arr):.4f} kcal/mol")
    print(f"  Temperature:")
    ...
    output_dir = Path("dhfr_md_ani2x_output")
    ...
    # Save energies
    np.savetxt(output_dir / "energies_total.txt", energies_total_arr,
               header="Total Energy ANI-2x+GB (kcal/mol)", fmt="%.6f")
    ...
    with open(output_dir / "summary.txt", 'w') as f:
        ...
        f.write("ANI-2x + GB Implicit Solvent\n")
        f.write("=" * 50 + "\n\n")
        ...
        f.write(f"\nEnergy Statistics:\n")
        f.write(f"  Total mean (ANI-2x+GB): {np.mean(energies_total_arr):.4f} kcal/mol\n")
        f.write(f"  Total std: {np.std(energies_total_arr):.4f} kcal/mol\n")
        f.write(f"\nTemperature Statistics:\n")
        f.write(f"  Mean: {np.mean(temperatures_traj):.2f} K\n")
        f.write(f"  Std: {np.std(temperatures_traj):.2f} K\n")
        f.write(f"\nRMSD:\n")
        f.write(f"  Final: {rmsd[-1]:.4f} Å\n")
        f.write(f"  Max: {np.max(rmsd):.4f} Å\n")

    print("  trajectory.xyz")
    print("  energies_total.txt")
